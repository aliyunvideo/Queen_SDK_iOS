...
/// @return 成功返回PIXELAI_OK， 失败返回其他错误码
PIXELAI_SDK_API pixelai_result_t
pixelai_dl_face3DFaceBeauty_Run_Anchor(pixelai_handle_t handle,
                         const std::vector<std::array<float, 2> > &input_pts,
                         std::vector<float>& orgArr,
...
/// ******************** PixelAI 美妆类型切换接口 ********************
/// @param[in] type makeup type
/// @return 成功返回PIXELAI_OK， 失败返回其他错误码
PIXELAI_SDK_API pixelai_result_t
pixelai_dl_face3DFaceMakeup_setMakeupType(pixelai_handle_t handle,
                                  const ALMMakeupType type
                                  );
...
/// @return 成功返回PIXELAI_OK， 失败返回其他错误码
PIXELAI_SDK_API pixelai_result_t
pixelai_dl_face3DFaceMakeup_create_online(
                                      pixelai_handle_t *handle,
                                      const std::string& model_Path = "",
                                      const pixelai_istream_func& func = nullptr
                                    );
